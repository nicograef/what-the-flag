...
function challengesRouter(jwtService) {
  ...
  // Get challenge by id
  router.get('/:challengeId', jwtService.validateJwt.bind(jwtService), async (req, res) => {
    try {
  ...
  router.post('/', [jwtService.validateJwt.bind(jwtService)], async (req, res) => {
    ...
    try {
      let challengedUser = await User.findOne({ username: req.body.username })
      if (!challengedUser)
        return res.status(404).json({
          errors: [
            {
  ...
  router.post('/:challengeId', [jwtService.validateJwt.bind(jwtService)], async (req, res) => {
    if (!req.body.answers) {
    ...
    try {
      ...
      if (req.body.answers.length !== challenge.questions.length)
        return res.status(400).json({
          errors: [
            {
              param: 'answers',
      ...
      // Calculate result
      const result = req.body.answers.map((answer, index) => (answer === challenge.questions[index].answer ? 1 : 0))
