  ...
  try {
    ...
    if (!userChallenged && !userWasChallenged)
      return res
        ...
        .json({
          errors: [
            { param: "userId", msg: "You are not part of this challenge." },
          ],
        });
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    challenges = challenges.map(({ id, from, to, createdAt, results }) => {
      ...
      else if (to.id === req.userId)
        challenge.from = `${from.emoji}${from.username}`;
      ...
      if (challenge.status === "completed")
        challenge.points = userResult.points;
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const userAnswers = challenge.results.find(
        (result) => result.user.toString() === req.userId,
      );
      if (userAnswers)
      ...
      if (challenge.results.length === 2) {
        ...
        const userToAnswer = challenge.results.find(
          (result) => result.user.toString() === userTo.id,
        );
